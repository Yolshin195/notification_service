...
from notification.message_sender_api import (
    message_sender_api,
    MessageSenderApiException,
)
...
def task_create_messages(dispatch_id: UUID):
    ...
    clients = Client.objects.filter(
        tag__id=dispatch.filter_tag.id,
    ...
    for client in clients:
        ...
        message = Message.objects.create(
            client=client, dispatch=dispatch, status=status
        )
        task_send_message.apply_async(args=[message.id])
        logger.info(
            f"task_create_messages: Send message task initiated for client: {client}"
        )
...
def task_send_message(self: Task, message_id: UUID):
    ...
    if message.status.code == MessageStatusEnum.COMPLETED.value:
        ...
        return
    if message.dispatch.end_datetime.astimezone(message.client.timezone) < datetime.now(
        message.client.timezone
    ):
        logger.info("task_send_message: Message sending time has passed.")
        message.status = MessageStatusReference.objects.get(
            code=MessageStatusEnum.TIMEOUT.value
        )
        message.save()
        return
    ...
    try:
        ...
        message.status = MessageStatusReference.objects.get(
            code=MessageStatusEnum.ERROR.value
        )
        message.save()
        raise self.retry(exc=exc, countdown=self.request.retries * 60)

    message.status = MessageStatusReference.objects.get(
        code=MessageStatusEnum.COMPLETED.value
    )
    message.save()
